...
server.route(
  /^\/site\.git(\/.*)?$/,
  gitServer(fixPath('site/site.git'), commitHandler(fixPath('site'), 'site.git', 'root', config.siteRoot))
)

/* Static site server */
server.route(
  /.*/,
  serveStatic(fixPath('site/root'), {
    dotfiles: 'ignore'
  })
)
...
server.route(
  /.*/,
  fileServer(fixPath('site/root'), config.errorPages['404'], 404)
)
