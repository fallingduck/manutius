...
module.exports = function(configPath) {
  ...
  return function(req, res, next) {
    ...
    if (whitelist[user.name]) {
      if (whitelist[user.name][0] === req.connection.remoteAddress && whitelist[user.name][1] === req.connection.remotePort && whitelist[user.name][2] > Math.floor(new Date() / 1000)) {
        return next()
      } else {
    ...
    checkAuth(configPath, user.name, user.pass, function(result) {
      if (result) {
        ...
        // Add to whitelist
        whitelist[user.name] = [req.connection.remoteAddress, req.connection.remotePort, Math.floor(new Date() / 1000) + 15]
